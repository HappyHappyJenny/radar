...
void delayUs(uint16_t time) {
	//hal_delay = ms >> us delay making function
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) {
	if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
		if (IsFirstCaptured == 0) //Rising edge
				{
			//Read timer (CNT value read)
			IC_Val1 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1);
			IsFirstCaptured = 1;
			//make next edge to Falling edge
			__HAL_TIM_SET_CAPTUREPOLARITY(htim, TIM_CHANNEL_1,
					TIM_INPUTCHANNELPOLARITY_FALLING);
		} else if (IsFirstCaptured == 1) //Falling edge
				{
			...
			//calculate with ultra sonic velocity
...
uint32_t getDistance() {
	//give trigger to get distance data
